...
const PaperCard: React.FC<PaperCardProps> = ({ paper, index, className }) => {
  ...
  return (
    <div 
      className={`group relative bg-[#303134] rounded-2xl p-6 shadow-sm border border-[#5f6368]/40 hover:shadow-lg hover:border-[#e8eaed]/50 transition-all duration-300 ease-out animate-fade-in-up ${className || ''}`}
      style={{ animationDelay: `${index * 100}ms` }}
    >
      <div className="flex flex-col h-full justify-between">
        <div>
          {/* Badge for Context/Reason */}
          <div className="inline-block px-3 py-1.5 rounded-xl text-xs font-medium bg-[#202124] text-[#e8eaed] border border-[#5f6368]/50 mb-3 whitespace-normal break-words max-w-full leading-relaxed">
            {paper.reason}
          </div>
